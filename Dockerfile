...
RUN git clone https://github.com/nvidia/caffe.git /caffe -b 'caffe-0.15' && \
    ...
    mkdir build && \
    cd build && \
    cmake -DCMAKE_INSTALL_PREFIX=/usr/local/caffe -DUSE_NCCL=ON -DUSE_CUDNN=ON -DCUDA_ARCH_NAME=Manual -DCUDA_ARCH_BIN="20 30 35 50 52 53 60 61" -DCUDA_ARCH_PTX="61" .. && \
    make -j"$(nproc)" install && \
    rm -rf /caffe
...
RUN pip install numpy==1.11.0 six==1.10.0
